...
void RIFF::File::read()
{
  ...
  while(tell() + 8 <= length()) {
    ...
    if(!isValidChunkID(chunkName)) {
	  ...
	  if(chunkName.size()==4 && chunkName[0]==0 &&  chunkName[1]==0 &&  chunkName[2]==0 &&  chunkName[3]==0)
	  {
		debug("chunkName is zero");
	  }
	  else
	  {
      	setValid(false);
    ...
    if(static_cast<ulonglong>(tell()) + chunkSize > static_cast<ulonglong>(length())) {
	  ...
	  if(chunkName=="UITS" || chunkName=="ID3 " || chunkName=="APPL"){
	  	debug("chunkName is UITS");
	  }
	  else {
	    setValid(false);
    	break;
