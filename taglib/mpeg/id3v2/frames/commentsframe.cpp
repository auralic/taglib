...
void CommentsFrame::parseFields(const ByteVector &data)
{
  ...
  if(l.size() == 2) {
    if(d->textEncoding == String::Latin1) {
	  // Get desc
	  ByteVector desctmp;
	  if(LocaleHelper::GBK2UTF8(l.front(), &desctmp))
	  {
	    d->description = String(desctmp, String::UTF8);
	  }
	  else
	  {
      ...
      // Get text 
      ByteVector texttmp;
      if(LocaleHelper::GBK2UTF8(l.back(), &texttmp))
      {
        d->text = String(texttmp, String::UTF8);
      }
      else
      {
